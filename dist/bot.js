...
const pendingPhotos = new Map(); // userId -> fileId
...
bot.command("kommunal", async (ctx) => {
    ...
    await ctx.reply(`💡 **Kommunal to'lov qo'shildi!**\n` +
        `Jami: ${amount.toLocaleString()} so'm\n` +
        `Har bir kishi uchun: **${splitAmount.toLocaleString()}** so'mdan taqsimlandi. ✅`);
});
// /start command
bot.command("start", async (ctx) => {
    await ctx.reply("👋 Xush kelibsiz! Men kvartira boshqaruvchisi botman.\n\n" +
        "🧹 **NAVBATCHILIK:**\n" +
...
bot.command("leave", async (ctx) => {
    ...
    if (myDebts.length > 0 || myCredits.length > 0) {
        ...
        if (myDebts.length > 0)
            warning += `- Sizdan qarzdorliklar mavjud.\n`;
        if (myCredits.length > 0)
            warning += `- Boshqalar sizdan qarzdor.\n`;
...
bot.command("join", async (ctx) => {
    if (ctx.chat.type === "private")
        return await ctx.reply("Guruhda ishlating.");
    ...
    if (!added)
        return await ctx.reply(`${firstName}, siz allaqachon qo'shilgansiz.`);
...
bot.command("list", async (ctx) => {
    ...
    assignments.forEach((a) => {
        ...
        if (a.isDebt)
            text += " (Eski qarzdan ⏳)";
        if (taskDebt > 0 && !a.isDebt)
            text += ` (Jami qarz: ${taskDebt + 1})`;
        else if (taskDebt > 0 && a.isDebt)
            text += ` (Qolgan qarz: ${taskDebt})`;
        text += "\n";
    });
...
bot.command("xarajat", async (ctx) => {
    const args = ctx.match.split(" ");
    if (args.length < 2)
        return await ctx.reply("Format: `/xarajat 50000 non`", { parse_mode: "Markdown" });
    const amount = parseInt(args[0]);
    const description = args.slice(1).join(" ");
    if (isNaN(amount) || amount <= 0)
        return await ctx.reply("Miqdor xato.");
    const groupId = ctx.chat.id.toString();
    const group = db.getGroups().find(g => g.id === groupId);
    if (!group || group.members.length < 2) {
        return await ctx.reply("Guruhda kamida 2 kishi bo'lishi kerak.");
...
bot.on("callback_query:data", async (ctx) => {
    ...
    if (data === "split_manual_init") {
        const session = pendingExpenses.get(userId);
        if (!session)
            return await ctx.answerCallbackQuery("Eski sessiya.");
        ...
        await ctx.editMessageText(`✍️ **Manual taqsimlash boshlandi.**\n\n` +
        ...
        if (session && group) {
            ...
            for (const m of group.members)
                if (m.userId !== userId)
                    await db.addDebt(userId, m.userId, split, session.description, session.groupId);
            pendingExpenses.delete(userId);
        ...
        if (myDebts.length === 0)
            return await ctx.editMessageText("Qarz topilmadi.");
        const keyboard = new InlineKeyboard();
        myDebts.forEach(d => {
...
bot.command("qarzlar", async (ctx) => {
    ...
    if (debts.length === 0)
        return await ctx.reply("Qarzlar yo'q. ✨");
    // Group debts by debtorId
    const debtorGroups = {};
    debts.forEach(d => {
        if (!debtorGroups[d.debtorId])
            debtorGroups[d.debtorId] = [];
    ...
    for (const debtorId in debtorGroups) {
        ...
        debtorDebts.forEach(d => {
            if (!payerGroups[d.payerId])
                payerGroups[d.payerId] = [];
...
bot.command("toladim", async (ctx) => {
    ...
    const myDebts = db.getDebts().filter(d => d.groupId === groupId && d.debtorId === userId && d.status === 'pending');
    if (myDebts.length === 0)
        return await ctx.reply("Sizda hozircha qarz yo'q. ✨");
    // Get unique payers
    const payerIds = [...new Set(myDebts.map(d => d.payerId))];
    const keyboard = new InlineKeyboard();
    payerIds.forEach(pId => {
...
async function getGeneralStatus(groupId) {
    const group = db.getGroups().find(g => g.id === groupId);
    if (!group)
        return null;
    ...
    if (assignments.length > 0) {
        ...
        assignments.forEach((a) => {
            ...
            text += `${a.task.label}: **${a.user?.firstName || "Noma'lum"}**`;
            if (a.isDebt)
                text += ` (Qarzdan: ${taskDebt})`;
            else if (taskDebt > 0)
                text += ` (Jami: ${taskDebt + 1})`;
            text += "\n";
        });
    }
    else {
    ...
    group.members.slice(0, 5).forEach(m => {
        const user = db.getUsers().find(u => u.id === m.userId);
...
cron.schedule("0 10 2 * *", async () => {
    ...
    for (const group of groups) {
        try {
            await bot.api.sendMessage(Number(group.id), "📢 **DIQQAT! BUGUN TO'LOV KUNI!** 💰\n\n" +
