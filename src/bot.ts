...
bot.command("list", async (ctx) => {
  const groupId = ctx.chat.id.toString();
  const group = db.getGroups().find((g) => g.id === groupId);
  if (!group || group.members.length === 0) return await ctx.reply("Ro'yxat bo'sh. /join qiling.");
...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (data === "split_manual_init") {
    ...
    if (chore && chore.userId !== userId && chore.status === 'pending') {
      ...
      } else await ctx.editMessageText("❌ Rad etildi.");
    }
  }
...
cron.schedule("0 0 * * *", async () => {
  ...
  for (const group of groups) {
    ...
    if (db.getChores().filter(c => c.groupId === group.id && c.status === 'confirmed' && c.createdAt.startsWith(yesterdayStr)).length === 0) {
      ...
      if (cur) try { await bot.api.sendMessage(Number(group.id), `⚠️ **JARIMA!**\n${cur.user.firstName} kecha ish qilmadi: +1 kun jarima.`); } catch {}
    }
  }
}, { timezone: "Asia/Tashkent" });
...
times.forEach(t => cron.schedule(t, async () => {
  ...
  for (const group of groups) {
    const cur = await getCurrentTurnUser(group.id);
    if (cur) try { await bot.api.sendMessage(Number(group.id), `🔔 **NAVBTCHI:** [${cur.user.firstName}](tg://user?id=${cur.userId}) 🧹🗑🧴\nIshni tugatib rasm + /done yuboring.`, { parse_mode: "Markdown" }); } catch {}
  }
}, { timezone: "Asia/Tashkent" }));
